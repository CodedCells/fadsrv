...
def check_existing_user(days, diff, user):
    logging.info('')
    logging.info(f'{diff}\t{days}')
    
    asked, user_count = gather_stats(user, force=True)
    
    if user_count > 11000:
        logging.info(f'User count: {user_count:,}, avoiding uneceesary trafic\n')
        return True


def check_existing_users(u):
    t = len(u)
    logging.info(f'Gather stats for {t:,} users')
    c = 0
    for days, diff, user in sorted(u, reverse=True):
        c += 1
        if check_existing_user(days, diff, user) == True:
            break
        
        delay = 2
        if c % 100 == 0:
            logging.info(f'{c:,} of {t:,}')
            delay = 10
...
if __name__ == '__main__':
    ...
    init_logger('user_stats_update', disp=':' in code_path)
